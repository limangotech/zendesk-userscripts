...
1. **prepare** — guards (tag matches the manifest version, version not already in the published feed), then creates the GitHub Release as a **draft**. Drafts are invisible to `releases/latest`, so users keep getting the previous release until everything is ready.
